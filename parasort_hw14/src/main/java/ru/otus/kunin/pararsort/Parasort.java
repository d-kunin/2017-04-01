...
public class Parasort {
  ...
  public static <T extends Comparable<? super T>> List<T> system(List<T> list) {
    ArrayList<T> result = Lists.newArrayList(list);
    Collections.sort(result);
    return result;
  }

  public static <T extends Comparable<? super T>> List<T> stream(List<T> list) {
    return list.parallelStream()
        .sorted()
        .collect(toList());
  }

  public static <T extends Comparable<? super T>> List<T> arraysParallel(List<T> list) {
    if (list.isEmpty()) {
      return list;
    }
  ...
  public static <T extends Comparable<? super T>> List<T> customSortWithExecutor(List<T> list) {
  ...
  public static <T extends Comparable<? super T>> List<T> customSortWithNewThreads(List<T> list) {
    final int chunkSize = calculateChunkSize(list);
